var fetchDataFromKernel = function() {
    return;
}

var _initialize = async function () {
    // TODO: change to local files src/img/spark-logo.jpg and src/css/spark-logo.css
...
var updateUIPreKernel = async function() {
    // initialize spark logo
    UI.sparkLogo.id = "sparkmanager_sparkLogo";
    // UI.sparkLogo.classList.add("btn");
    // UI.sparkLogo.classList.add("btn-default");
    UI.sparkLogo.src = "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f3/Apache_Spark_logo.svg/1200px-Apache_Spark_logo.svg.png";
    UI.sparkLogo.style = '\
        height: 33px;\
        width: 60px;\
        margin-left: 10px;\
    ...
    UI.startButton.id = "sparkmanager_startButton";
    UI.startButton.classList.add("btn");
    UI.startButton.classList.add("btn-default");
    // add "Start" text
    var startButtonText = document.createElement("span");
    startButtonText.id = "sparkmanager_startButtonText";
    startButtonText.classList.add("toolbar-btn-label");
    startButtonText.innerHTML = "Start";
    UI.startButton.appendChild(
        startButtonText
    );
    UI.startButton.onclick = startStopButtonHandler;
    
    // initialize cluster select drop down 
    UI.clusterSelect.id = "sparkmanager_clusterSelect";
    UI.clusterSelect.classList.add("form-control");
    UI.clusterSelect.classList.add("select-xs");
    // UI.clusterSelect.style = "height: 23px; width: 90px; font-size: 14px";
    UI.clusterSelect.onchange = clusterSelectHandler;

    // cluster status
    UI.clusterStatus.id = "sparkmanager_clusterStatus";
    UI.clusterStatus.classList.add("btn");
    UI.clusterStatus.classList.add("btn-default");
    var clusterStatusText = document.createElement("span");
    clusterStatusText.id = "sparkmanager_clusterStatusText";
    clusterStatusText.classList.add("toolbar-btn-label");
    clusterStatusText.innerHTML = "Cores: N/A | Memory: N/A";
    UI.clusterStatus.appendChild(
        clusterStatusText
    );

    // Config button
    UI.applicationConfig.innerHTML = `<img id ="spark_config_logo" src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Windows_Settings_app_icon.png/480px-Windows_Settings_app_icon.png" style="height: 16px;"/>`;
    UI.applicationConfig.classList.add("btn");
    UI.applicationConfig.classList.add("btn-default");

    UI.sparkUI.id = "sparkmanager_sparkUI";
    UI.sparkUI.classList.add("btn");
    UI.sparkUI.classList.add("btn-default");
    var sparkUIText = document.createElement("span");
    sparkUIText.id = "sparkmanager_sparkUIText";
    sparkUIText.classList.add("toolbar-btn-label");
    sparkUIText.innerHTML = "UI";

    UI.sparkUI.appendChild(
        sparkUIText
    );
    ...
    UI.container.appendChild(
        UI.applicationConfig
    );
    UI.container.appendChild(
        UI.sparkUI
    );
    
    UI.container.id = "sparkmanager_ui";
    // UI.container.style = "display: inline;";
